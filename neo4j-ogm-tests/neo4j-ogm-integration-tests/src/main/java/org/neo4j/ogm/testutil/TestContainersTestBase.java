...
public class TestContainersTestBase {
    ...
    static {
        ...
        String tugraphUrl = Optional.ofNullable(System.getenv(SYS_PROPERTY_NEO4J_URL)).orElse("");
        String tugraphPassword = Optional.ofNullable(System.getenv(SYS_PROPERTY_NEO4J_PASSWORD)).orElse("").trim();
        if (!(tugraphUrl.isEmpty() || tugraphPassword.isEmpty()) && isRpcDriver()) {
            LOGGER.info("Using TuGraph instance at {}.", tugraphUrl);
            ...
            baseConfigurationBuilder = new Configuration.Builder()
                ...
                .credentials("admin", tugraphPassword);
            driver.configure(baseConfigurationBuilder.build());
            version = "";
            isEnterpriseEdition = false;
        } else if (!(tugraphUrl.isEmpty() || tugraphPassword.isEmpty()) && isBoltDriver()) {
            LOGGER.info("Using Neo4j instance at {}.", tugraphUrl);
            driver = new BoltDriver();
            baseConfigurationBuilder = new Configuration.Builder()
                .uri(tugraphUrl)
                .verifyConnection(true)
                .withCustomProperty(BoltDriver.CONFIG_PARAMETER_BOLT_LOGGING, Logging.slf4j())
                .credentials("neo4j", tugraphPassword);
            driver.configure(baseConfigurationBuilder.build());
    ...
    protected static boolean isRpcDriver() {
        return transport == Transport.RPC;
    }
